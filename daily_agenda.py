...
Usage:
    daily_agenda.py append AGENDA.md [--name "Daily agenda"] [--folder /]
    daily_agenda.py render AGENDA.md          # print the page text, no upload
...
Requirements:
  ...
  - reMarkable device token: either an existing ~/.rmapi config, or the env
    ...
    ~/.rmapi on first run.
  - python package `rmscene` (pip install rmscene) for writing .rm v6 pages.
...
def cmd_append(args: argparse.Namespace) -> None:
    text = markdown_to_page_text(Path(args.markdown).read_text())
    rm = page_rm_bytes(text)
    ensure_token()
    binary = find_rmapi()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if got.returncode == 0 and bundle.exists():
            ...
            print(f"downloaded existing notebook ({content.get('pageCount')} pages)", file=sys.stderr)
        else:
            doc_id, content, meta, files = new_bundle(args.name)
            print("notebook not found; creating it", file=sys.stderr)
